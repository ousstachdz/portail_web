...
class OpportunityControllerAPI(http.Controller):
    ...
    def opportunity_submit(self, **post):
        try:
                ...
                if key.startswith('document_') and base64_file:
                    try:
                        ...
                        if isinstance(base64_file, str) and base64_file.strip():
                            ...
                            logger.exception(f'Invalid base64 string provided for document {document_id}')
                    except (ValueError, AttributeError) as e:
                        logger.exception(f'Error processing document {key}: {e}')
                        continue
            ...
            if user:
                logger.exception('User with this login already exists')
            else:
    ...
    def get_opportunities(self, **kwargs):
        # Fetch all pw.lead records created in the last 5 minutes
        recent_time = fields.Datetime.now() - timedelta(minutes=5000)
        all_opportunities = request.env['pw.lead'].sudo().search([('date_creation', '>=', recent_time)])
                        ...
                        'apropos': [
                            {
                                ...
                                'age': str(partner.age),
                                'pourcentage': str(partner.pourcentage),
                        ...
                        'documents': [
                            {
                                ...
                                'document': str(doc.document) if doc.document else '',
