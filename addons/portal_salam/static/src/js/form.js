  ...
  static template = xml`
      <div class="w-100">
        ...
        <t t-if="state.data">
            ...
            <t t-else="">
            <h3 style="color: #071530;">املاء البيانات التالية</h3>
          ...
          <div  t-attf-class="row {{ (state.step === 3) ? '' : 'd-none' }} w-100">
            <Form3 state="state"/>
          </div>
          <div  t-attf-class="row {{ (state.step === 4) ? '' : 'd-none' }}">
            <Form4 state="state"/>
          </div>
          <div  t-attf-class="row {{ (state.step === 500) ? '' : 'd-none' }}">
...
<div class="container mt16 mb16">
  <div dir="rtl py-4">
    <div class="alert alert-danger text-center my-4" role="alert">
      ...
      <p>
        يرجى التحقق من اتصال الإنترنت والتأكد من صحة البيانات المدخلة.
      </p>
      <hr>
      </hr>
      <p class="mb-0">
        في حال استمرار الخطأ، يرجى التواصل مع البنك عبر البريد الإلكتروني التالي:
        <br></br>
        <strong>support@alsalamalgeria.com</strong>
      </p>
    </div>
  </div>
</div>
