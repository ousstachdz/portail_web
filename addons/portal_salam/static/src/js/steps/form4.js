class Form4 extends Component {
  ...
  //       if (file.size > max_size * MB) {
  //         this.state.files_errors[name + '_error'] = `حجم الملف يجب أن لا يتجاوز ${max_size} ميغابايت.`
  //         state.formData[name] = null
  //         event.target.files = null
  //         return
  //       }
  ...
  async handleChange(event) {
    ...
    try {
      ...
      // Save only document ID
      state.formData[name] = result.id
      console.log(state.formData)
    } catch (error) {
      console.error(error)
      this.state.files_errors[name + '_error'] = 'حدث خطأ أثناء رفع الملف.'
      state.formData[name] = null
      event.target.value = ''
    }
  }
  ...
  static template = xml`
<label for="documents" style="font-size: 16px; font-weight:bold; color: #F2643B;">
